...
export default function SongBrowser({ api: providedApi, onReview, outputSettings, onOpenOutputSettings, onOutputDirChange }) {
  ...
  const currentSelection = { backingTrack: 'any', backingStrict: false, instrumentRequirements: [], parts: searchState.searchedRequest?.filters.parts || [], partsMatch: searchState.searchedRequest?.filters.partsMatch || 'all', tuning: searchState.searchedRequest?.filters.tuning || null, platform: 'pc', strictPlatform: true };
      ...
      {!available ? (
        ...
        <form className="sb-search-form" onSubmit={(event) => { event.preventDefault(); search(query); }}>
          ...
          <div className="sb-filter-grid">
            ...
            <label>Arrangement match<select aria-label="Arrangement match" value={searchState.filters.partsMatch} onChange={(event) => searchSession.setFilters({ partsMatch: event.target.value })}><option value="any">Any selected (OR)</option><option value="all">All selected (AND)</option></select></label>
          </div>
          ...
          <p>Any selected matches at least one checked arrangement; All selected requires every checked arrangement. Leave all unchecked for no arrangement filter.</p>
        ...
        <div className="sb-content-grid">
          <section className="sb-results" aria-label="Search results" aria-busy={searching}>
            ...
            {results.length ? <><div className={`sb-result-list ${searching ? "sb-searching" : ""}`}>{results.map((song) => <ResultCard key={String(song.id)} song={song} job={latestJobs.get(String(song.id))} busy={busy.has(`enqueue:${song.id}`)} canDownload={outputReady && !searching} onDownload={(id) => downloadSong(id)} onReviewAnother={(id) => downloadSong(id, true)} suitability={typeof api?.assessResult === 'function' && assessmentEntries.some((entry) => entry.id === String(song.id)) ? assessments.scope === assessmentScope ? assessments.results[String(song.id)] || { pending: true, reusable: false } : { pending: true, reusable: false } : undefined} onShowOutput={showOutput} selected={selected.has(String(song.id))} onSelect={selectChart} />)}</div><nav className="sb-pagination" aria-label="Search results pages"><button type="button" className="sb-button" disabled={page <= 1 || searching} onClick={() => searchSession.changePage(page - 1)}><ChevronLeft size={16} aria-hidden="true" /> Previous</button><span>Page {page}</span><button type="button" className="sb-button" disabled={!searchResult.hasNext || searching} onClick={() => searchSession.changePage(page + 1)}>Next <ChevronRight size={16} aria-hidden="true" /></button></nav></> : <div className="sb-empty"><div className="sb-empty-icon">{searching ? <LoaderCircle size={28} className="sb-spin" aria-hidden="true" /> : <Search size={28} aria-hidden="true" />}</div><h3>{searching ? "Looking for your song…" : challenge || needsSignIn ? "Complete your connection" : searchError || error ? "Search needs attention" : searchResult ? "No charts found" : "Start with an artist or a song"}</h3><p>{searching ? "Results will appear here." : challenge || needsSignIn ? "Finish the step in the browser window, then run your search again." : searchError || error ? "Check the message above and try again." : searchResult ? `No results for “${searchedQuery}”. Try another title or a shorter artist name.` : "Compare chart versions, instruments and tunings before adding a song."}</p></div>}
          </section>
